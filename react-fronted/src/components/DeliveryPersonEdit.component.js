...
class deliveryPersonEdit extends Component {
  ...
  async componentDidMount() {
    if (this.props.match.params.username != 'new') {
      const deliveryPerson = await (await fetch(`/api/redis/DeliveryPerson/getById/${this.props.match.params.username}`)).json();
                ...
                <FormGroup>
                  <FormGroup>
                    {this.state.disabledId == true ?
                      <div>
                        <FormGroup>
                          <Label for="statosCurrentlyWorking">Busy status</Label>{' '}
