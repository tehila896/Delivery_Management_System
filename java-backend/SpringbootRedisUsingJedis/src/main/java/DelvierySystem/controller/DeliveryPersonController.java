...
public class DeliveryPersonController {
	...
	// Url - http://localhost:10091/api/redis/DeliveryPerson/add_DeliveryPerson
	@PostMapping("/add_DeliveryPerson")
	public ResponseEntity<String> save(@Valid @RequestBody final DeliveryPerson DeliveryPerson) {
		try {
			DeliveryPerson temp = serviceDeliveryMen.findById(DeliveryPerson.getUsername());
			if (temp.getUsername() == null)
				return null;
		} catch (Exception ex) {
			  UsersRole newUser=new UsersRole(DeliveryPerson.getUsername(),Role.ROLE_DELIVERY_PERSON,DeliveryPerson.getPassword());
	...
	// Url - http://localhost:10091/api/redis/DeliveryPerson/update_State/state/{state}/id/{id}
