...
public class DeliveryPersonService implements DeliveryPersonRepo{
	...
	public Boolean calclute_Distance(Package deliveryPackage) {
		Double distancePackage = distance_Between_LatLong(deliveryPackage.getP_source().getX(),
				deliveryPackage.getP_source().getY(), deliveryPackage.getP_destninon().getX(),
				deliveryPackage.getP_destninon().getY());
		...
		for (DeliveryPerson item : map_DeliveryMens.values()) {
			if (!(item.isStatosCurrentlyWorking()) && (item.getLock().equals("unlock"))
					&& (item.getCarryingCapacity().getValue() >= deliveryPackage.getCarryingCapacity().getValue())) {
				Double distanceWay = (distance_Between_LatLong(deliveryPackage.getP_source().getX(),
						deliveryPackage.getP_source().getY(), item.getPosition().getX(), item.getPosition().getY())
						+ distancePackage);
				if (distanceWay <= 5)
					unitOfDistance = UnitOfDistance.FIVE_KM;
				else if (distanceWay <= 15)
		 ...
		 * The package ID is sent to wait in Kafka at the end of the queue until a
		 * suitable deliveryPerson is updated
		 */
		if (map_temp.size() == 0) {
			return false;
		}
		...
		Double distanceWay = (distance_Between_LatLong(deliveryPackage.getP_source().getX(),
				deliveryPackage.getP_source().getY(), deliveryMen.getPosition().getX(),
				deliveryMen.getPosition().getY()) + distancePackage);
		if (distanceWay <= 5)
